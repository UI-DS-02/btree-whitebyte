...
public class ArrayBPTree01<K, V> extends AbstractBPTree<K, V> {
    ...
    public static class ExternalNode<K, V> extends Node<K, V> {
        ...
        public ExternalNode(int m, K k, V v) {  // insert dp in map and then pass it here
            super(m - 1, (int) (Math.ceil(m / 2.0) - 1), 0);
            this.map = new MapEntry[m];
            this.insertExternal(k, v);  //
        }
        ...
        public boolean insertExternal(K k, V v) {
        ...
        public void deleteExternal1(Comparator<K> keyComp, ExternalNode<K, V> exn, InternalNode<K, V> root, Comparator<Entry<K, V>> comp) {
            ...
            if (this.leftSibling != null &&
                ...
                this.insertExternal(borrowedDP.getKey(), borrowedDP.getValue());  /////////////
                this.sortMap(comp);
    ...
    public ArrayBPTree01(int m, Comparator<K> comp, K k) {
        super(comp);
        this.m = m;
        this.root = null;
        this.key = k;
    ...
    public Entry<K, V> insert(K key, V value) {
        ...
        if (isEmpty()) {
            ...
            this.firstEx = new ExternalNode<K, V>(this.m, key, value);

        } else {

            // Find leaf node to insert into
            ExternalNode<K, V> ln = (this.root == null) ? this.firstEx :
                    findExternalNode(key);

            // Insert into leaf node fails if node becomes overfull
            if (!ln.insertExternal(key, value)) {
